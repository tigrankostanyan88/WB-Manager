...
export function SocialLinks({ settings }: SocialLinksProps) {
  ...
  return (
    <div className="relative p-6 rounded-[1.5rem] bg-gradient-to-br from-white/60 to-white/30 backdrop-blur-xl shadow-lg shadow-violet-100/50 ring-1 ring-white/80">
        ...
        {visibleLinks.map((social) => {
          ...
          return (
            <a
              ...
              className={`group relative flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r ${social.gradient} ${social.shadow} shadow-lg text-white hover:shadow-xl hover:scale-105 transition-all duration-300 overflow-hidden`}
            >
              {/* Shine effect */}
              ...
              <span className="font-medium text-sm relative z-10">{social.label}</span>
            </a>
          )
        })}
      </div>
