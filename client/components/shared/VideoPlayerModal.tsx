...
export function VideoPlayerModal({ videoUrl, onClose, className = '' }: VideoPlayerModalProps) {
  ...
  return (
    ...
    >
      <div
        className={`relative w-full max-w-5xl max-h-[85vh] mx-auto bg-black flex flex-col items-center justify-center ${className}`}
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          className="absolute -top-12 right-0 z-[10000] w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition-colors"
