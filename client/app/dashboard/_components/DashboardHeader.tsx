...
export default function DashboardHeader({ activeTab, userSearch, setUserSearch, studentSearch, setStudentSearch }: DashboardHeaderProps) {
  return (
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
      ...
      <div className="flex items-center gap-3">
        <div className="relative">
          ...
          <input
            type="text"
            value={activeTab === 'students' ? studentSearch : activeTab === 'users' ? userSearch : ''}
            onChange={(e) => {
              const v = e.target.value
              if (activeTab === 'students') setStudentSearch(v)
              else if (activeTab === 'users') setUserSearch(v)
            }}
            placeholder="Փնտրել..."
